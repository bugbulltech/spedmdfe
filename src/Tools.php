...
require_once 'Auxiliar/Identify.php';
require_once 'Auxiliar/Response.php';
...
public function sefazEnviaLote(
  ...
  $this->urlService = trim($this->urlService);
...
public function sefazCancela(
  ...
  $retorno = $this->zSefazEvento($siglaUF, $chMDFe, $cOrgao, $tpAmb, $tpEvento, $nSeqEvento, $tagAdic, $dhEvento);
...
public function sefazConsultaNaoEncerrados($tpAmb = '2', $cnpj = '', &$aRetorno = array())
{
  ...
  $this->urlService = trim($this->urlService);
  $cons = "<consMDFeNaoEnc xmlns=\"$this->urlPortal\" versao=\"3.00\">"
  . "<tpAmb>$tpAmb</tpAmb>"
  ...
  //montagem dos dados da mensagem SOAP
  $body = "<mdfeDadosMsg xmlns=\"$this->urlNamespace\">$cons</mdfeDadosMsg>";
  $this->urlHeader = str_replace("3.0","3.00",$this->urlHeader);
  //consome o webservice e verifica o retorno do SOAP
  $retorno = $this->oSoap->send(
    $this->urlService,
    $this->urlNamespace,
...
public function validarXml($xml = '')
{
  ...
  if (! ValidXsd::validar($aResp['xml'], $xsdPath)) {
    ...
    return ['error' => true, 'errors' => $this->errors];
  }
  return ['success' => true];
}
}
