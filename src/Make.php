...
class Make extends BaseMake
{
  ...
  */
  ...
  /**
  * tagSeg
  * Identificação do seguro da MDFe [118] pai 0
  ...
  */
  public function tagSeg(
    ...
    $identificador = '[118] <seg> - ';
    $this->seg = $this->dom->createElement('seg');

    $this->dom->appChild($this->seg, $this->infResp, 'Falta tag "seg"');
    $this->dom->appChild($this->seg, $this->infSeg, 'Falta tag "seg"');
    ...
    if(!empty($nAver)){
      foreach ($nAver as $key => $value) {
        ...
        $this->dom->addChild(
          ...
          $value,
          false,
          $identificador . "Número da averbação"
        );
  ...
  */
  public function tagInfResp(
    ...
    $identificador = '[119] <infResp> - ';
    $this->infResp = $this->dom->createElement('infResp');
    $this->dom->addChild(
      $this->infResp,
      "respSeg",
      $respSeg,
    ...
    if($CNPJ){
      $this->dom->addChild(
        ...
        $CNPJ,
        false,
        $identificador . "Número do CNPJ do responsável pelo seguro"
      );
    }
    if($CPF){
      $this->dom->addChild(
        ...
        $CPF,
        false,
        $identificador . "Número do CPF do responsável pelo seguro"
      );
    }
    return $this->infResp;
  }
  ...
  */
  public function tagInfSeg(
    ...
    $identificador = '[123] <infSeg> - ';
    $this->infSeg = $this->dom->createElement('infSeg');
    $this->dom->addChild(
      $this->infSeg,
      "xSeg",
      $xSeg,
    ...
    $this->dom->addChild(
      ...
      $CNPJ,
      false,
      $identificador . "Número do CNPJ da seguradora"
    );
    return $this->infSeg;
  }
...
public function tagInfANTT(
  ...
  $this->dom->appChild($this->infANTT, $this->infContratante, 'Falta tag "infANTT"');
  return $this->infANTT;
}
...
/**
* tagInfContratante
* tag MDFe/infMDFe/infModal/rodo/tagInfContratante
*
* @param  string $RNTRC
* @return DOMElement
*/
public function tagInfContratante(
  ...
  if($CNPJ){
    $this->dom->addChild(
      ...
      $CNPJ,
      false,
      "Número do CNPJ do contratente do serviço"
    );
  }
